...
Releases on `main` are automated with [release-please](https://github.com/googleapis/release-please): merge Conventional Commit messages (`feat:`, `fix:`, etc.), then merge the release PR when you are ready to tag. When a GitHub Release is created, CI builds an unsigned `.xpi` via `web-ext build` and attaches it to that release, then submits that build to [AMO](https://addons.mozilla.org/) for signing ([`kewisch/action-web-ext`](https://github.com/kewisch/action-web-ext) with `channel: listed`). No separate source archive is submitted — the `.xpi` contains unminified, human-readable source that AMO can review directly. The workflow declares **GPL-3.0-or-later** via AMO's **`custom_license`** metadata (name + full `LICENSE` text), because Mozilla's `version.license` field only allows [fixed slugs](https://mozilla.github.io/addons-server/topics/api/licenses.html) which omit `GPL-3.0-or-later`. If AMO returns a signed file (some listed submissions finish asynchronously), that signed `.xpi` is uploaded to the GitHub Release as well. AMO signing runs in a **separate Actions job** that downloads the unsigned `.xpi` from a workflow artifact, so you can **re-run only that failed job** after transient errors without rebuilding.
