...
/**
 * Visible link label: trimmed title if non-empty, otherwise URL.
 * @param {string} title
 * @param {string} url
 * @returns {string}
 */
function linkLabelText(title, url) {
  const t = title ?? "";
  return t.trim() ? t : (url ?? "");
}
...
function markdownFormat(title, url) {
  const t = linkLabelText(title, url);
  const u = url ?? "";
  return `- [${t}](${u})`;
}
...
function htmlLinkFormat(title, url) {
  const label = escapeHtml(linkLabelText(title, url));
...
function formatHtmlUlLinks(tabs) {
  const lines = tabs.map((tab) => {
    const t = escapeHtml(linkLabelText(tab.title ?? "", tab.url ?? ""));
    const u = escapeHtml(tab.url ?? "");
